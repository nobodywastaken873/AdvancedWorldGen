...
public class RandomGems : ControlledWorldGenPass
{
	...
	protected override void ApplyPass()
	{
        ...
        for (int index = 0; index < Main.maxTilesX * stoneGemsRate; ++index)
        {
            ...
            if (!Main.tile[i, j].HasTile && !(Main.tile[i, j].LiquidType == LiquidID.Lava) && !Main.wallDungeon[(int)Main.tile[i, j].WallType] && Main.tile[i, j].WallType != (ushort)27)
            {
        ...
        for (int index280 = 0; index280 < Main.maxTilesX * desertGemsRate; ++index280)
        {
            ...
            if (!Main.tile[index281, index282].HasTile && !(Main.tile[index281, index282].LiquidType == LiquidID.Lava) && (Main.tile[index281, index282].WallType == (ushort)216 || Main.tile[index281, index282].WallType == (ushort)187))
            {
